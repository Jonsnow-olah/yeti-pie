...
function parseBytecode(moduleName, bytes) {
  ...
  console.log(`\nModule: ${moduleName} (Magic: 0x${magic.toString(16).toUpperCase()}, Version: ${version}, Tables: ${tableCount})`);
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (len === 1) {
      ...
      decoded = `String/Bytes: "${str.replace(/[^a-zA-Z0-9_\-\s:\/]/g, '.')}" (len: ${len}, hex: ${rawVal.toString('hex')})`;
    }
    console.log(`  Constant #${i}: Type Token: ${typeToken}, ${decoded}`);
  }
}
