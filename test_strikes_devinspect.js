...
async function main() {
  const specificStrikes = [65000, 63798, 63532, 60996, 61600, 64208, 64002];
  console.log('Testing specific strikes...');
  for (const strike of specificStrikes) {
    const res = await testStrike(strike);
    console.log(`Strike: ${strike} -> Success: ${res.success}${res.success ? '' : `, Error: ${res.error}`}`);
  }

  console.log('\nTesting a range around spot (63763)...');
  const rangeStrikes = [];
  for (let s = 63500; s <= 64000; s += 50) {
    rangeStrikes.push(s);
  }
  for (const strike of rangeStrikes) {
    const res = await testStrike(strike);
    console.log(`Strike: ${strike} -> Success: ${res.success}${res.success ? '' : `, Error: ${res.error}`}`);
  }
}
