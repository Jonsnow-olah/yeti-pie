...
async function testOracle(oid, expiry, initialSharedVersion, strike, isAbove) {
  ...
  const oracleArg = tx.sharedObjectRef({ objectId: oid, initialSharedVersion, mutable: false });
  const clockArg = tx.sharedObjectRef({ objectId: '0x6', initialSharedVersion: 1, mutable: false });

  const marketKey = tx.moveCall({
    target: PACKAGE_ID + '::market_key::new',
    arguments: [
      tx.pure(bcs.Address.serialize(oid).toBytes()),
      tx.pure(bcs.u64().serialize(BigInt(expiry)).toBytes()),
  ...
  try {
    ...
    if (res.error) {
      ...
      return { success: true, price: premium / 1000000 };
    } else {
      return { success: false, error: res.result.effects.status.error };
    }
  } catch (err) {
...
async function run() {
  try {
    ...
    console.log(`Testing top active candidate oracles...`);

    for (let i = 0; i < Math.min(5, activeBtcOracles.length); i++) {
      const bestOracle = activeBtcOracles[i];
      ...
      console.log(`\nCandidate ${i+1}: ${bestOracle.oracle_id} | Spot: ${spot} | On-chain Active: ${active} | Expiry: ${bestOracle.expiry}`);
      
      const res = await testOracle(bestOracle.oracle_id, bestOracle.expiry, initialSharedVersion, 63000, false);
      if (res.success) {
        console.log(`  -> Strike 63000 Below: Success, Price: ${res.price.toFixed(4)}`);
      } else {
        console.log(`  -> Strike 63000 Below: Failed, Error: ${res.error}`);
      }
    }
  } catch (err) {
    console.error(err);
