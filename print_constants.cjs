...
function parseBytecode(bytes) {
  ...
  const [tableCount, tableCountRead] = readUleb128(bytes, pos);
  pos += tableCountRead;
  ...
  for (let i = 0; i < tableCount; i++) {
    ...
    const [offset, offsetRead] = readUleb128(bytes, pos);
    pos += offsetRead;
    const [length, lengthRead] = readUleb128(bytes, pos);
    pos += lengthRead;
  ...
  const [count, countRead] = readUleb128(bytes, pos);
  pos += countRead;
  console.log(`\nConstant pool size: ${count} constants\n`);
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      if (len === 1) {
        ...
        console.log(`  As U16: ${rawVal.readUInt16LE(0)}`);
      } else if (len === 4) {
        console.log(`  As U32: ${rawVal.readUInt32LE(0)}`);
      } else if (len === 8) {
...
async function main() {
  ...
  try {
    ...
    // Look at constants module
    const predictBytecodeBase64 = bcsData.moduleMap['constants'];
    if (!predictBytecodeBase64) {
      console.log('constants module not found');
      return;
    }
